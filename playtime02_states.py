...
#~Where did my classmates get their abbreviations? liststates.com doesn't have abbreviations. I fumbled with 'with open("states.csv", "r") as states_file:        
    #states = states_file.read().split("\r")' but couldn't separate Abb from State as they were coupled elements in each list item 
...
# At line 14 ~or 43~, we create the drop-down menu
# At line 15 ~or 44~, we create one drop-down item.  Each additional <option> that we add will add another item to our drop-down menu
# At line 16 ~or 45~, we tell HTML that we're done with the drop-down menu

print "<select>" #you can print an html tag from python
for state, abb in statesAndAbbs_List: 
